...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        // CI/runtime-contract mode: run the exact in-process backend without
        // creating the WinForms/WebView2 window. Normal launches never use this.
        if (args.Contains("--headless", StringComparer.OrdinalIgnoreCase))
        {
    ...
    // Keep browser back/forward mouse buttons from accidentally navigating the
    // embedded WebView2 history away from the SleepyChat application surface.
